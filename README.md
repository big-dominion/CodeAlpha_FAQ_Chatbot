...
- **main.py**: This is the front door of the whole app. It starts the FastAPI server, serves the frontend page, and defines the three API routes (/api/v1/chat, /api/v1/tts, /api/v1/translate). It also checks that the required secret keys are set before it will even start, and keeps a short term memory of each conversation.
